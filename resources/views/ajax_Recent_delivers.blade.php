    ...
    <table class="table text-md-nowrap  text-center our-table" id="SearchProductTable" width="100%" style="border: 2px solid rgba(0,0,0,.3);">
        ...
        <tbody>
            ...
            <tr id="<?php echo $product['id']; ?>">
                <td dir="ltr" data-target="id">{{ $i}}</td>
                <td dir="ltr" data-target="id">{{ $product->supllier->name??'' }}</td>
                <td data-target="numberofpice">{{round($product->blance,2)}}</td>
                <td>
                    <div class="d-flex justify-content-center">
                                    <form action="{{ url(Mcamara\LaravelLocalization\Facades\LaravelLocalization::getCurrentLocale() . '/' . ($page = 'print_delivery_to_anoter_supplier')) }}" method="POST" role="search" autocomplete="off">
                                        ...
                                        <div class="d-flex justify-content-center">
                                            <input type="number" class="form-control parent-input " name="OrderNoprint" id="OrderNoprint" title=" رقم الفاتورة "  value="{{$product->to_dlivery_id}}"  hidden>
